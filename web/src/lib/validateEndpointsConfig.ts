/**
 * Client-side copy of [`electron/mock-api/validation.cjs`](electron/mock-api/validation.cjs)
 * so import merges can be checked before persisting (same rules as the mock server).
 */
import type { EndpointConfig, HttpMethod } from '../types'
import { sanitizePathSegment } from './sanitizePath'

const SAMPLE_JSON_MAX_CHARS = 120_000
const SAMPLE_JSON_MAX_DEPTH = 40
const SAMPLE_JSON_MAX_KEYS = 2000
const ALLOWED_METHODS = new Set<HttpMethod>(['GET', 'HEAD', 'POST', 'PUT', 'PATCH', 'DELETE'])

function countJsonKeys(
  node: unknown,
  depth: number,
...
function resolveEndpointMethod(ep: EndpointConfig, defaultMethod: HttpMethod): HttpMethod {
  const d = ALLOWED_METHODS.has(defaultMethod) ? defaultMethod : 'GET'
  if (ep.method && ALLOWED_METHODS.has(ep.method)) {
    return ep.method
  }
  return d
}

function validateEndpointExamples(
...
export function validateEndpointsConfig(
  ...
  const dm: HttpMethod = ALLOWED_METHODS.has(defaultMethod) ? defaultMethod : 'GET'
  const seenRoutes = new Set<string>()
  for (const ep of config as EndpointConfig[]) {
